...
if __name__ == "__main__":
    ...
    value = torch.randn(batch, seq_len, d_model, dtype=torch.double, requires_grad=True, device=device)

    W_q = torch.randn(d_model, d_model, dtype=torch.double, requires_grad=True, device=device)
    W_k = torch.randn(d_model, d_model, dtype=torch.double, requires_grad=True, device=device)
